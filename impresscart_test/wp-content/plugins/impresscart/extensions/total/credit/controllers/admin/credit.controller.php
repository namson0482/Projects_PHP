<?php
class impresscart_admin_credit_controller extends impresscart_framework_controller {
	...
	function setting() {
		...
		$this->data['heading'] = __('Total credit setting');
		
		if($_SERVER['REQUEST_METHOD'] == 'POST') {				
			$result = $this->getNewSetting($_POST);			
			$this->model_total_credit->update_setting($result);			
			$this->session->data['success'] = __('Total credit setting updated successfully');
			$this->redirect('admin.php?page=total');				
		}

		$current_setting = $this->model_total_credit->get_setting();


		$this->data['heading_title'] = __('Credit');
		...
		'href'      => 'admin.php?page=total', 
      	...
      	if ($current_setting['status'] == 'yes') {
      		$this->data['credit_status'] = true;
      	} else {
      		$this->data['credit_status'] = false;
      	}
      	$this->data['credit_order'] = $current_setting['order'];
